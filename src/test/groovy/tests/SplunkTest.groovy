...
class SplunkTest extends DataAnalyticsSteps{
    ...
    void violationTypesOverTimeStats() {
        ...
        Response response = splunk.splunkSearchResults(splunk_username, splunk_password, splunkBaseURL, search_string)
        def licenseViolationCount = response.jsonPath().getList("results").stream().reduce(0, (int count, data) -> count + Integer.parseInt(data["License"]))
        def securityViolationCount = response.jsonPath().getList("results").stream().reduce(0, (int count, data) -> count + Integer.parseInt(data["Security"]))
        def expected = SplunkSteps.getExpectedViolationCounts(license_issues, security_issues)
        def expectedSecurityViolations = expected.remove("security") ?: 0
        def expectedLicenseViolations = expected.values().sum()

        Assert.assertEquals(licenseViolationCount, expectedLicenseViolations)
        Assert.assertEquals(securityViolationCount, expectedSecurityViolations)
