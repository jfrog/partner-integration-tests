...
class SplunkTest extends DataAnalyticsSteps{
    ...
    void watchesCountTest() throws Exception {
        Object[][] license_issues = xraySteps.multipleLicenseIssueEvents()

        def search_string = 'search=search log_source="jfrog.xray.siem.vulnerabilities" | ' +
                'stats dc(signature) as watches '
        Response createSearch = splunk.createSearch(splunk_username, splunk_password, splunkBaseURL, search_string)
        ...
        if (size == 0){
            ...
            Assert.assertTrue(requests >= license_issues.length + 1)
    ...
    void vulnerabilitiesCountTest() throws Exception {
        Object[][] security_vulnerabilities = xraySteps.multipleIssueEvents()
        Object[][] artifacts = xraySteps.artifacts()
        def vulnerability_count = security_vulnerabilities.length * artifacts.length

        def search_string = 'search=search log_source="jfrog.xray.siem.vulnerabilities" category="Security" earliest=-4h ' +
        ...
        if (size == 0){
            ...
            println(requests)
            // One watch per license in multipleLicenseIssueEvents and one watch for all security violations
            Assert.assertTrue(requests >= vulnerability_count)
    ...
    void licenseIssuesCountTest() throws Exception {
        Object[][] license_issues = xraySteps.multipleLicenseIssueEvents()
        Object[][] artifacts = xraySteps.artifacts()
        def license_issues_count = license_issues.length * artifacts.length

        def search_string = 'search=search log_source="jfrog.xray.siem.vulnerabilities" category="License" | stats count as licenses'
        Response createSearch = splunk.createSearch(splunk_username, splunk_password, splunkBaseURL, search_string)
        createSearch.then().statusCode(201)
        def searchID = splunk.getSplunkSearchID(splunk_username, splunk_password, splunkBaseURL, search_string)
        ...
        if (size == 0){
            ...
            println(requests)
            // One watch per license in multipleLicenseIssueEvents and one watch for all security violations
            Assert.assertTrue(requests >= license_issues_count)
    ...
    void violationsCountTest() throws Exception {
        ...
        def search_string = 'search=search log_source="jfrog.xray.siem.vulnerabilities"| stats count as violations'
        Response createSearch = splunk.createSearch(splunk_username, splunk_password, splunkBaseURL, search_string)
        createSearch.then().statusCode(201)
        def searchID = splunk.getSplunkSearchID(splunk_username, splunk_password, splunkBaseURL, search_string)
        ...
        if (size == 0){
            ...
            println(requests)
            // One watch per license in multipleLicenseIssueEvents and one watch for all security violations
            Assert.assertTrue(requests >= vulnerability_count + license_issues_count)
    ...
    void infectedComponentsCountTest() throws Exception {
        Object[][] artifacts = xraySteps.artifacts()
        def infected_components_count = artifacts.length

        def search_string = 'search=search log_source="jfrog.xray.siem.vulnerabilities"| ' +
                ' stats dc(infected_components{}) as infected_components'
        Response createSearch = splunk.createSearch(splunk_username, splunk_password, splunkBaseURL, search_string)
        createSearch.then().statusCode(201)
        def searchID = splunk.getSplunkSearchID(splunk_username, splunk_password, splunkBaseURL, search_string)
    ...
    void impactedArtifactsCountTest() throws Exception {
        Object[][] artifacts = xraySteps.artifacts()
        def impacted_artifacts_count = artifacts.length

        def search_string = 'search=search log_source="jfrog.xray.siem.vulnerabilities"| ' +
                ' stats dc(impacted_artifacts{}) as impacted_artifacts'
        Response createSearch = splunk.createSearch(splunk_username, splunk_password, splunkBaseURL, search_string)
        createSearch.then().statusCode(201)
        def searchID = splunk.getSplunkSearchID(splunk_username, splunk_password, splunkBaseURL, search_string)
        ...
        if (size == 0){
            ...
            println(requests)
            // One watch per license in multipleLicenseIssueEvents and one watch for all security violations
            Assert.assertTrue(requests >= impacted_artifacts_count)
    ...
    void violationsPerWatchTest() throws Exception {
        ...
        def search_string = 'search=search log_source="jfrog.xray.siem.vulnerabilities" earliest=-24h | stats count by signature'
        Response createSearch = splunk.createSearch(splunk_username, splunk_password, splunkBaseURL, search_string)
        createSearch.then().statusCode(201)
        def searchID = splunk.getSplunkSearchID(splunk_username, splunk_password, splunkBaseURL, search_string)
        ...
        if (size == 0){
            ...
            for (def i in 0..(license_issues.length - 2)) {
                response.then().assertThat().statusCode(200)
                        .body("results[" + i + "].signature", startsWith('test_'))
                JsonPath jsonPathEvaluator = response.jsonPath()
                def count = jsonPathEvaluator.getInt("results[" + i + "].count")
                Assert.assertTrue((count >= 1))
    ...
    void violationsSeverityTest() throws Exception {
        def search_string = 'search=search log_source="jfrog.xray.siem.vulnerabilities" severity!=Unknown | stats count by severity'
        Response createSearch = splunk.createSearch(splunk_username, splunk_password, splunkBaseURL, search_string)
        createSearch.then().statusCode(201)
        def searchID = splunk.getSplunkSearchID(splunk_username, splunk_password, splunkBaseURL, search_string)
        ...
        if (size == 0){
            ...
            for (def i in 0..2) {
                response.then().assertThat().statusCode(200)
                        .body("results[" + i + "].severity", isIn("High", "Medium", "Low"))
                JsonPath jsonPathEvaluator = response.jsonPath()
                def count = jsonPathEvaluator.getInt("results[" + i + "].count")
                Assert.assertTrue((count >= 1))
    ...
    void violationsByPolicyTest() {
        ...
        Response response = splunk.splunkSearchResults(splunk_username, splunk_password, splunkBaseURL, search_string)
        def actual = SplunkSteps.getMatchedPolicyCounts(response)
