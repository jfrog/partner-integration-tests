...
class DataAnalyticsSteps extends TestSetup{
    ...
    def http201(count, calls){
        while (count <= calls) {
            ...
            def passwordRt = config.artifactory.fake_password
    ...
    def http403(count, calls){
        while (count <= calls) {
            def repoName = "generic-dev-local"
            Response http403 = repoSteps.deleteRepository(artifactoryURL, repoName, "user1", config.artifactory.fake_password)
